...
export const ProspectDetailsPopup = ({
	...
	const handleConvertToClient = async () => {
		...
		try {
			...
			if (result.success && result.data?.id) {
				onProspectConverted?.(result.data.id);
				onClose();
				router.push(`/dashboard/admin/clients/${result.data.id}`);
				return;
			}
	...
	const selectedOffer = prospect.selectedOffer;
	const isCreditsEligible = selectedOffer
		? isSmallGroupCreditsEligible(selectedOffer.program.type)
		: false;
	const smallGroupCredits =
		isCreditsEligible && prospect.selectedSmallGroupCredits != null
			? Math.max(0, prospect.selectedSmallGroupCredits)
			: 0;
	const offerPricing = selectedOffer
		? calculateSmallGroupPricing(selectedOffer.price, smallGroupCredits)
		: null;
		...
		>
			<div className="bg-gray-800 border border-gray-600 rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
				...
				<div className="p-3 sm:p-4 space-y-4">
					...
					<div className="flex items-center space-x-3">
						...
						<div className="flex-1 min-w-0">
							...
							<a
								...
								className="block text-white underline hover:text-blue-400 transition-colors break-all"
							>
								{prospect.email}
							</a>
						</div>
					...
					{prospect.weight && (
						<div className="flex items-center space-x-3">
							<Weight className="w-5 h-5 text-blue-400 shrink-0" />
							<div className="flex-1 min-w-0">
					...
					{/* Selected Offer */}
					{selectedOffer && offerPricing && (
						<div className="flex items-start space-x-3">
							<Package className="w-5 h-5 text-green-400 mt-1 shrink-0" />
							<div className="flex-1 min-w-0">
								<span className="text-gray-400 text-sm">Offre souhaitée</span>
								<div className="space-y-2 mt-1">
									<div className="bg-gray-700 rounded-lg p-3 space-y-2">
										<h4 className="text-white font-medium text-sm sm:text-base">
											{selectedOffer.program.name}
										</h4>
										<div className="grid grid-cols-2 gap-2 text-sm">
											...
											<div>
												...
												<span className="text-white ml-2">
													{selectedOffer.duration} mois
												</span>
											</div>
											<div>
												<span className="text-gray-400">
													<span className="sm:hidden">Crédits SG:</span>
											...
											<div className="col-span-2">
												<span className="text-gray-400">Prix Package:</span>
												<span className="text-white font-medium ml-2">
													{formatPrice(offerPricing.totalMonthlyPrice)}
												</span>
											</div>
										</div>
									</div>
