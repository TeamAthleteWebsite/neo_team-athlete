...
import { Calendar, Clock, Repeat, RotateCcw, X } from "lucide-react";
...
export const AddSessionPopup: React.FC<AddSessionPopupProps> = ({
	isOpen,
	onClose,
	clientId,
	contractId,
		...
		try {
			if (isRecurringEnabled && isRecurrenceValid()) {
				...
				if (!response.ok) {
					throw new Error(
						result.error ||
							"Erreur lors de la création des séances récurrentes",
					);
				}
                    ...
                    `}
							...
							{selectedDays.length > 0 && (
								<div className="text-white/60 text-xs">
									Jours sélectionnés:{" "}
									{selectedDays
										.map((id) => daysOfWeek.find((d) => d.id === id)?.fullName)
										.join(", ")}
								</div>
							)}
