...
export const PaymentTab: React.FC<PaymentTabProps> = ({ plannings }) => {
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        <div className="text-white/60 text-lg">Chargement des paiements...</div>
      </div>
    );
  }
  ...
  if (monthlyData.length === 0) {
    return (
      <div className="text-center py-12">
        <div className="text-white/60 text-lg">Aucun contrat trouvé</div>
      </div>
    );
  }
        ...
        return (
          ...
          >
            ...
            <div 
              ...
              onClick={() => !monthData.isPaid && !isProcessing && handleMonthClick(monthData)}
            >
              <div className="flex items-center gap-4">
                ...
                {monthData.isPaid && monthData.paymentDate && (() => {
                  const payment = payments.find(p => p.id === monthData.paymentId);
                  const paymentDate = payment?.createdAt ? new Date(payment.createdAt).toLocaleDateString('fr-FR') : 'Date inconnue';
